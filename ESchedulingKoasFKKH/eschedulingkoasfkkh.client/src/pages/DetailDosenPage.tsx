...
export default function DetailDosenPage() {
  ...
  return (
    <Layout>
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-emerald-600 to-green-700 rounded-2xl p-6 sm:p-7 text-white shadow-xl mb-6 animate-fade-in-down print:hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2 self-start md:self-center">
            ...
            >
              <PrintIcon className="w-4 h-4 text-emerald-200" />
              <span>Cetak PDF</span>
            </button>
            ...
            {isAdmin && dosen && (
              <>
                ...
                <button
                  ...
                  className="px-4 py-2.5 bg-white hover:bg-emerald-50 active:scale-95 text-emerald-800 font-bold rounded-xl shadow-md transition-all duration-200 text-sm flex items-center gap-2 cursor-pointer"
                >
      ...
      {loading ? (
        <div className="bg-white rounded-2xl shadow-card border border-slate-100 p-16 text-center">
          ...
          <p className="text-slate-600 font-medium">Memuat data lengkap dosen...</p>
          <p className="text-xs text-slate-400 mt-1">Mengambil profil, stase pengampuan, dan riwayat bimbingan</p>
        </div>
      ) : !dosen ? (
        <div className="bg-white rounded-2xl shadow-card border border-slate-100 p-16 text-center">
          <div className="w-16 h-16 rounded-full bg-slate-100 text-slate-400 flex items-center justify-center mx-auto mb-4">
            <DosenIcon className="w-8 h-8" />
          </div>
          <h2 className="text-lg font-bold text-slate-800">Dosen Tidak Ditemukan</h2>
          <p className="text-sm text-slate-500 mt-1 mb-6">Data dosen dengan ID tersebut tidak tersedia di sistem.</p>
          <button
            onClick={() => navigate('/dosen')}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-card border border-slate-100/80 p-6 overflow-hidden relative">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
              <div className="flex items-center gap-5">
                ...
                <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-emerald-500 via-teal-600 to-green-600 flex items-center justify-center text-white text-3xl font-bold shadow-lg shadow-emerald-500/20 border-2 border-white">
                ...
                <div>
                  <div className="flex flex-wrap items-center gap-2 mb-1.5">
                    ...
                    {dosen.koordinatorStase && dosen.koordinatorStase.length > 0 && (
                      <span className="text-xs font-bold px-2.5 py-1 rounded-lg bg-amber-50 text-amber-700 border border-amber-200 flex items-center gap-1">
                  ...
                  <h2 className="text-2xl font-bold text-primary-900 tracking-tight">{dosen.nama}</h2>
              ...
              <div className="w-full md:w-auto p-4 rounded-xl bg-gradient-to-br from-slate-50 to-emerald-50/50 border border-slate-200/80 min-w-[240px]">
                <p className="text-xs font-semibold uppercase text-slate-400 tracking-wider mb-1 flex items-center gap-1.5">
                  <StaseIcon className="w-4 h-4 text-emerald-500" />
                  Stase Pengampuan
                </p>
                {dosen.daftarStase && dosen.daftarStase.length > 0 ? (
                  <div className="flex flex-wrap gap-1.5 mt-1">
                    {dosen.daftarStase.map(stase => (
                      <span
                        key={stase}
                        className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-emerald-100/70 text-emerald-800 border border-emerald-200/70"
                      >
                        {stase}
                      </span>
                    ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white rounded-2xl shadow-card border border-slate-100/80 p-5 flex items-center gap-4 hover:shadow-md transition-shadow">
              ...
              <div>
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Stase Diampu</p>
                <div className="flex items-baseline gap-1.5">
                  <span className="text-2xl font-bold text-slate-800">{dosen.statistik?.totalStaseDiampu || 0}</span>
                  <span className="text-xs text-slate-400 font-medium">Stase Klinis</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-card border border-slate-100/80 p-5 flex items-center gap-4 hover:shadow-md transition-shadow">
              ...
              <div>
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Bimbingan Aktif</p>
                <div className="flex items-baseline gap-1.5">
                  <span className="text-2xl font-bold text-slate-800">{dosen.statistik?.totalBimbinganAktif || 0}</span>
                  <span className="text-xs text-slate-400 font-medium">Kelompok</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-card border border-slate-100/80 p-5 flex items-center gap-4 hover:shadow-md transition-shadow">
              ...
              <div>
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Bimbingan Mendatang</p>
                <div className="flex items-baseline gap-1.5">
                  <span className="text-2xl font-bold text-slate-800">{dosen.statistik?.totalBimbinganMendatang || 0}</span>
            ...
            <div className="bg-white rounded-2xl shadow-card border border-slate-100/80 p-5 flex items-center gap-4 hover:shadow-md transition-shadow">
              ...
              <div>
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Selesai</p>
                <div className="flex items-baseline gap-1.5">
                  <span className="text-2xl font-bold text-slate-800">{dosen.statistik?.totalBimbinganSelesai || 0}</span>
                  <span className="text-xs text-slate-400 font-medium">Histori Arsip</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="border-b border-slate-200/80 flex items-center gap-2 print:hidden">
            ...
            >
              ...
              <span>Jadwal Bimbingan Aktif & Terjadwal</span>
            ...
            >
              ...
              <span>Riwayat Bimbingan Selesai (Arsip)</span>
            ...
            <div className="bg-white rounded-2xl shadow-card border border-slate-100/80 overflow-hidden">
              ...
              {!dosen.jadwalBimbingan || dosen.jadwalBimbingan.length === 0 ? (
                <div className="p-12 text-center">
                  ...
                  <h4 className="text-base font-semibold text-slate-800">Tidak Ada Jadwal Bimbingan Aktif</h4>
          ...
          {(activeTab === 'riwayat' || typeof window === 'undefined') && (
            <div className="bg-white rounded-2xl shadow-card border border-slate-100/80 overflow-hidden">
              <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-gradient-to-r from-purple-50/50 to-indigo-50/30">
                <div>
                  <h3 className="text-base font-bold text-primary-900 flex items-center gap-2">
              ...
              {!dosen.riwayatBimbingan || dosen.riwayatBimbingan.length === 0 ? (
                <div className="p-12 text-center">
                  ...
                  <h4 className="text-base font-semibold text-slate-800">Belum Ada Riwayat Bimbingan</h4>
                  <p className="text-xs text-slate-500 max-w-md mx-auto mt-1">
                ...
                <div className="p-12 text-center">
                  <p className="text-sm font-medium text-slate-600">Tidak ada riwayat bimbingan yang cocok dengan pencarian "{historySearch}"</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full min-w-max">
              ...
              <div className="bg-white rounded-2xl shadow-card border border-slate-100/80 p-6">
                <div className="flex items-center gap-3 mb-4 pb-3 border-b border-slate-100">
                  <div className="w-10 h-10 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center border border-amber-200">
                    <KoordinatorIcon className="w-5 h-5" />
                  </div>
                  <div>
                    <h3 className="text-base font-bold text-primary-900">Koordinator Stase</h3>
                    <p className="text-xs text-slate-500">Stase yang dikoordinatori oleh dosen ini</p>
                  </div>
                </div>
                ...
                {!dosen.koordinatorStaseDetail || dosen.koordinatorStaseDetail.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {dosen.koordinatorStaseDetail.map(stase => (
                      <div
                        key={stase.id}
                        className="p-4 rounded-xl border border-amber-200/70 bg-gradient-to-r from-amber-50/40 to-orange-50/20 flex items-center justify-between"
                      >
                        <div className="flex items-center gap-3">
                          <span className="text-amber-500 text-lg">★</span>
                          <div>
                            <h4 className="text-sm font-bold text-primary-900">{stase.nama}</h4>
                            <p className="text-xs text-slate-500">Durasi: {stase.jumlahHari} hari kerja</p>
                          </div>
                        </div>
                        <button
                          onClick={() => navigate(`/stase/${stase.id}`)}
                          className="text-xs text-amber-700 hover:text-amber-900 font-semibold hover:underline cursor-pointer"
                        >
                          Lihat Stase →
                        </button>
                      </div>
              ...
              <div className="bg-white rounded-2xl shadow-card border border-slate-100/80 p-6">
                <div className="flex items-center gap-3 mb-4 pb-3 border-b border-slate-100">
                  <div className="w-10 h-10 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center border border-emerald-200">
                    <StaseIcon className="w-5 h-5" />
                  </div>
                  <div>
                    <h3 className="text-base font-bold text-primary-900">Stase yang Diampu</h3>
                    <p className="text-xs text-slate-500">Daftar stase tempat dosen ini terdaftar mengajar</p>
                  </div>
                </div>
                ...
                {!dosen.daftarStaseDetail || dosen.daftarStaseDetail.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {dosen.daftarStaseDetail.map(stase => (
                      <div
                        key={stase.id}
                        className="p-4 rounded-xl border border-emerald-200/70 bg-emerald-50/30 flex items-center justify-between"
                      >
                        <div className="flex items-center gap-3">
              ...
              <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                {selectedHistory.daftarMahasiswa && selectedHistory.daftarMahasiswa.length > 0 ? (
                  selectedHistory.daftarMahasiswa.map((mhs, idx) => (
                    <div
                      key={idx}
                      className="flex items-center justify-between p-3 rounded-xl border bg-slate-50 border-slate-200/80"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs font-bold text-slate-800">{mhs.nama}</p>
              ...
              <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                {selectedActiveJadwal.daftarMahasiswa && selectedActiveJadwal.daftarMahasiswa.length > 0 ? (
                  selectedActiveJadwal.daftarMahasiswa.map((mhs) => (
                    <div
                      key={mhs.id}
                      className="flex items-center justify-between p-3 rounded-xl border bg-emerald-50/40 border-emerald-200"
                    >
                      <div className="flex items-center gap-3">
      ...
      {showEditModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs animate-fade-in">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full overflow-hidden border border-slate-100 animate-scale-up">
            <div className="p-5 bg-gradient-to-r from-emerald-600 to-green-700 text-white flex items-center justify-between">
              <div>
                <h3 className="text-base font-bold flex items-center gap-2">
                  <EditIcon className="w-5 h-5" />
                  Edit Data Dosen
                </h3>
                <p className="text-xs text-emerald-100 mt-0.5">Perbarui informasi NIP atau nama dosen</p>
              </div>
              <button
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-400 focus:bg-white transition-all font-mono"
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-emerald-400 focus:bg-white transition-all"
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center justify-end gap-2">
                ...
                <button
                  ...
                  onClick={saveEdit}
                  disabled={savingEdit}
                  className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-xl transition-all cursor-pointer disabled:opacity-50 flex items-center gap-1.5"
                >
          ...
          >
            ...
            <div className="px-6 py-5 bg-gradient-to-r from-amber-600 to-amber-700 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-extrabold text-base text-white">Reset Password Dosen</h3>
                  <p className="text-xs text-amber-100">Atur ulang kata sandi akun dosen pembimbing</p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleResetPassword} className="p-6 space-y-4">
              ...
              <div className="p-3 bg-amber-50/70 border border-amber-200/80 rounded-xl text-xs space-y-1">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-slate-800">{dosen.nama}</span>
                </div>
                <div className="flex justify-between">

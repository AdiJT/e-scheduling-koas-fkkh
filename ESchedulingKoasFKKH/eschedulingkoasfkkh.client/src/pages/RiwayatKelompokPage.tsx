...
export default function RiwayatKelompokPage() {
  ...
  return (
    <Layout>
      {/* Page Header Card */}
      <div className="relative overflow-hidden bg-gradient-to-r from-indigo-800 to-indigo-700 rounded-2xl p-4 sm:p-6 text-white shadow-xl mb-4 sm:mb-6 animate-fade-in-down">
        {/* Subtle decorative watermark */}
        <div className="absolute -right-6 -bottom-8 opacity-10 pointer-events-none transform rotate-12">
          <HistoryIcon className="w-44 h-44 sm:w-56 sm:h-56 text-white" />
        </div>
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center text-white shadow-md shrink-0">
              <HistoryIcon className="w-5 h-5 sm:w-6 sm:h-6 text-indigo-200" />
            </div>
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-white tracking-tight">Riwayat Kelompok</h1>
              <p className="text-xs sm:text-sm text-indigo-100/90">Daftar lengkap stase yang telah selesai dijalani oleh kelompok KOAS</p>
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2.5 sm:gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 sm:pl-10 pr-4 py-2 sm:py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm focus:outline-none focus:border-indigo-400 transition-all"
            />
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-card border border-slate-100/80 overflow-hidden animate-fade-in-up">
        {loading ? (
          ...
          <>
            {/* Table Header Info */}
            <div className="px-4 sm:px-5 py-3 bg-slate-50/50 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 sm:gap-2">
            ...
            <div className="overflow-x-auto">
              <table className="w-full table-auto" id="table-riwayat">
                <thead>
                  <tr className="bg-gradient-to-r from-indigo-800 to-indigo-700 text-white">
                    ...
                    <th
                      ...
                      className="px-3.5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider whitespace-nowrap cursor-pointer select-none hover:bg-indigo-900/50 w-24"
                    >
                      Kelompok {renderSortIndicator('namaKelompok')}
                    </th>
                    <th
                      onClick={() => handleSort('tahunAjaran')}
                      className="px-3.5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider whitespace-nowrap cursor-pointer select-none hover:bg-indigo-900/50 w-32"
                    >
                      Tahun Ajaran {renderSortIndicator('tahunAjaran')}
                    </th>
                    <th
                      onClick={() => handleSort('namaStase')}
                      className="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider cursor-pointer select-none hover:bg-indigo-900/50"
                    >
                      Stase {renderSortIndicator('namaStase')}
                    </th>
                    <th
                      onClick={() => handleSort('tanggalMulai')}
                      className="px-3.5 py-3.5 text-center text-xs font-semibold uppercase tracking-wider whitespace-nowrap cursor-pointer select-none hover:bg-indigo-900/50 w-44"
                    >
                      Jadwal Stase {renderSortIndicator('tanggalMulai')}
                    </th>
                    <th className="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider">Dosen Pembimbing</th>
                    <th className="px-3.5 py-3.5 text-center text-xs font-semibold uppercase tracking-wider whitespace-nowrap w-28">Anggota</th>
                    <th className="px-3.5 py-3.5 text-center text-xs font-semibold uppercase tracking-wider whitespace-nowrap w-24">Aksi</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {paginatedData.map((r, index) => (
                    <tr key={r.id} className="hover:bg-indigo-50/20 transition-colors duration-150 group">
                      ...
                      <td className="px-4 py-3.5 font-semibold text-indigo-900 leading-snug break-words">
                        {r.namaStase}
                      </td>
                      ...
                      <td className="px-4 py-3.5">
                        {r.namaPembimbing ? (
                          <div className="inline-flex items-center gap-1.5 text-xs font-semibold text-teal-700 bg-teal-50 px-2.5 py-1 rounded-lg border border-teal-100 max-w-full">
                            <span className="shrink-0">👨‍🏫</span>
                            <span className="break-words leading-tight">{r.namaPembimbing}</span>
                          </div>
                          ...
                          <div className="inline-flex items-center gap-1.5 text-xs font-semibold text-indigo-700 bg-indigo-50 px-2.5 py-1 rounded-lg border border-indigo-100 max-w-full">
                            <span className="shrink-0">📦</span>
                            <span className="break-words leading-tight">Multi-Dosen Sub-Stase</span>
                          </div>
                        ) : (
                          <span className="text-xs text-slate-400 italic font-mono">- Tidak ada -</span>
                        )}
                      </td>
                      <td className="px-3.5 py-3.5 text-center whitespace-nowrap">
      ...
      {selectedRiwayat && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl mx-auto my-8 animate-scale-in overflow-hidden border border-slate-100 flex flex-col max-h-[90vh]">
            {/* Modal Header */}
            <div className="relative overflow-hidden bg-gradient-to-r from-indigo-800 via-indigo-700 to-primary-900 text-white p-4 sm:p-6 flex items-center justify-between shrink-0 shadow-md">
              {/* Subtle decorative watermark */}
              <div className="absolute -right-4 -bottom-6 opacity-10 pointer-events-none transform rotate-12">
                <KelompokIcon className="w-40 h-40 sm:w-48 sm:h-48 text-white" />
              </div>

              <div className="relative z-10 flex items-center gap-3 sm:gap-3.5">
                <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl sm:rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center text-white shadow-sm shrink-0">
                  <KelompokIcon className="w-5 h-5 sm:w-6 sm:h-6 text-indigo-200" />
                </div>
                <div>
              ...
              <button
                ...
                className="relative z-10 w-8 h-8 sm:w-9 sm:h-9 bg-white/10 hover:bg-white/20 active:scale-95 rounded-xl flex items-center justify-center font-bold transition-all text-white border border-white/20 cursor-pointer backdrop-blur-sm shrink-0"
                title="Tutup Modal"
              >
                ✕
              </button>
            ...
            {/* Modal Footer */}
            <div className="p-4 sm:p-5 border-t border-slate-100 bg-slate-50/60 flex justify-end shrink-0">
              <button
                onClick={() => setSelectedRiwayat(null)}
                className="w-full sm:w-auto px-6 py-2.5 bg-gradient-to-r from-indigo-800 to-indigo-900 hover:from-indigo-700 hover:to-indigo-800 text-white font-semibold rounded-xl shadow-md transition-all text-xs sm:text-sm cursor-pointer active:scale-95 text-center"
              >
                Tutup Detail
              </button>
            </div>
